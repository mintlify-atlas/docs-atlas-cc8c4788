...
description: "Run the MindFlow backend and frontend locally using Node.js workspaces, with a local or remote PostgreSQL instance and environment variables loaded from .env files."
---
...
<Steps>
  ...
  <Step title="Copy and configure environment files">
    ...
    <Note>
      `DATABASE_URL` must point to a PostgreSQL instance that is already running and reachable before you start the backend. The backend will retry the connection up to five times (with a 5-second pause between attempts), but the database must be accessible at startup.
    </Note>
  </Step>
