---
title: "Dashboard API: aggregated student data endpoint"
sidebarTitle: "Dashboard"
description: "GET /api/v1/dashboard returns the authenticated student's active tasks sorted by deadline, pending micro-objectives grouped by task, and the last 30 fatigue scores in chronological order."
---

The Dashboard endpoint is MindFlow's single-call aggregation surface. Rather than requiring the frontend to chain multiple requests for tasks, micro-objectives, and fatigue history, `GET /api/v1/dashboard` fetches all three datasets in a single parallel database query and returns them under a unified response envelope. This keeps the initial page load fast and makes polling straightforward.
...
Returns the authenticated student's aggregated dashboard data: active tasks (with their nested active micro-objectives) sorted by deadline, and the student's last 30 fatigue scores ordered chronologically.
...
<ResponseField name="data.fatigueHistory" type="array">
  Up to 30 fatigue records for the student, ordered by `recordedAtUtc` descending as returned from the database. Contains one entry per fatigue score submission. If the student has never submitted a score, this array is empty.
</ResponseField>
